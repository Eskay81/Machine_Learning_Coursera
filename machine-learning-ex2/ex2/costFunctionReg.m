...
% grad(1) = 1/m * X(1,1) * (sigmoid(X(1,:) * theta) - y(1) );
% J = 1/m * (-y' * log(sigmoid(X * theta)) - (1-y)' * log(1 - sigmoid(X * theta))) + lambda/m * theta' * theta;

grad_temp1 = 1/m * X' *(sigmoid(X*theta) - y);
grad_temp2  = 1/m * X' * (sigmoid(X * theta) - y ) + lambda/m * theta;
grad = [grad_temp1(1) ; grad_temp2(2:end)];
